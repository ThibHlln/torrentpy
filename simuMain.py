...
def main():
    ...
    time_steps = my__time_frame.get_list_datetime()
    ...
    for node in my__network.nodes:
        dict__data_frames[node] = DataFrame(index=time_steps, columns=my_variables).fillna(0.0)  # filled with zeros
    ...
    for link in my__network.links:
        ...
        dict__data_frames[link] = DataFrame(index=time_steps, columns=my_headers).fillna(0.0)  # filled with zeros
