...
def river_model(identifier, obj_network, waterbody, dict_data_frame,
    ...
    if identifier == "LINRES":
        linres.run(obj_network, waterbody,
                   dict_data_frame, dict_param, dict_meteo,
                   datetime_time_step, time_gap,
                   logger)


def lake_model(identifier, waterbody, dict_data_frame, dict_param, dict_meteo, datetime_time_step, time_gap, logger):
