...
def run(waterbody, dict_data_frame,
    ...
    if c_s_v_h2o_ove < 0.0:
        logger.info("{}: {} - Volume in OVE Store has gone negative, volume reset "
    ...
    if c_s_v_h2o_dra < 0.0:
        logger.info("{}: {} - Volume in DRA Store has gone negative, volume reset "
                    "to zero.".format(waterbody, datetime_time_step))
        c_s_v_h2o_dra = 0.0
    # route interflow
    ...
    if c_s_v_h2o_int < 0.0:
        logger.info("{}: {} - Volume in INT Store has gone negative, volume reset "
                    "to zero.".format(waterbody, datetime_time_step))
        c_s_v_h2o_int = 0.0
    ...
    if c_s_v_h2o_sgw < 0.0:
        logger.info("{}: {} - Volume in SGW Store has gone negative, volume reset "
                    "to zero.".format(waterbody, datetime_time_step))
        c_s_v_h2o_sgw = 0.0
    ...
    if c_s_v_h2o_dgw < 0.0:
        logger.info("{}: {} - Volume in DGW Store has gone negative, volume reset "
                    "to zero.".format(waterbody, datetime_time_step))
        c_s_v_h2o_dgw = 0.0
    ...
    # # 2.1. Collect inputs, states, parameters, and constants
    ...
    for store in stores:
        ...
        for contaminant in stores_contaminants:
            ...
            my_dict_4[contaminant] = dict_const["SMART"]["c_cst_mob_{}_{}".format(contaminant, store)]
    ...
    daily_sediment_threshold = dict_const["SMART"]['c_cst_sed_daily_thresh']  # [mm/day]
    sediment_threshold = daily_sediment_threshold * time_factor
    sediment_k = dict_const["SMART"]['c_cst_sed_k']
    sediment_p = dict_const["SMART"]['c_cst_sed_p']
    soil_test_p = dict_const["SMART"]['c_cst_soil_test_p']  # [kg/kg]
    day_growing_season = dict_const["SMART"]['c_cst_day_grow']
    day_of_year = float(datetime_time_step.timetuple().tm_yday)
    if calendar.isleap(datetime_time_step.timetuple().tm_year):
        days_in_year = 366.0
    else:
        days_in_year = 365.0
    cst_c1n = dict_const["SMART"]["c_cst_soil_c1n"]  # rate coefficient (m/day) for denitrification
    cst_c3n = dict_const["SMART"]["c_cst_soil_c3n"]  # rate coefficient (m/day) for NO3 plant uptake
    cst_c4n = dict_const["SMART"]["c_cst_soil_c4n"]  # rate coefficient (m/day) for nitrification
    cst_c5n = dict_const["SMART"]["c_cst_soil_c5n"]  # rate coefficient (m/day) for N mineralisation
    cst_c6n = dict_const["SMART"]["c_cst_soil_c6n"]  # rate coefficient (m/day) for N immobilisation
    cst_c7n = dict_const["SMART"]["c_cst_soil_c7n"]  # rate coefficient (m/day) for NH4 plant uptake
    cst_c1p = dict_const["SMART"]["c_cst_soil_c1p"]  # rate coefficient (m/day) for organic P plant uptake
    cst_c2p = dict_const["SMART"]["c_cst_soil_c2p"]  # rate coefficient (m/day) for P immobilisation
    cst_c3p = dict_const["SMART"]["c_cst_soil_c3p"]  # rate coefficient (m/day) for P mineralisation
    cst_c4p = dict_const["SMART"]["c_cst_soil_c4p"]  # conversion rate of r. avail. organic P into f. bound organic P
    cst_c5p = dict_const["SMART"]["c_cst_soil_c5p"]  # conversion rate of f. bound organic P into r. avail. organic P
    cst_c6p = dict_const["SMART"]["c_cst_soil_c6p"]  # rate coefficient (m/day) for inorganic P plant uptake
    cst_c7p = dict_const["SMART"]["c_cst_soil_c7p"]  # conversion rate of r. avail. ino. P into f. bound ino. P
    cst_c8p = dict_const["SMART"]["c_cst_soil_c8p"]  # conversion rate of f. bound ino. P into r. available ino. P
    ...
    for store in ['ove', 'dra']:
        ...
        for contaminant in ['no3', 'nh4', 'dph']:
            ...
            if (m_store < 0.0) or (dict_states_hd[store] < volume_tolerance):
                logger.info("{}: {} - {} Quantity in {} Store has gone negative, quantity reset "
    ...
    if (m_soil_new < 0.0) or ((lvl_total_end / 1e3 * area) < volume_tolerance):
        logger.info("{}: {} - NO3 Quantity in SOIL Store has gone negative, quantity reset "
                    "to zero.".format(waterbody, datetime_time_step))
    ...
    if (m_soil_new < 0.0) or ((lvl_total_end / 1e3 * area) < volume_tolerance):
        logger.info("{}: {} - NH4 Quantity in SOIL Store has gone negative, quantity reset "
                    "to zero.".format(waterbody, datetime_time_step))
    ...
    if (m_soil_new < 0.0) or ((lvl_total_end / 1e3 * area) < volume_tolerance):
        logger.info("{}: {} - P_INO_RA Quantity in SOIL Store has gone negative, quantity reset "
                    "to zero.".format(waterbody, datetime_time_step))
    ...
    if (m_soil_new < 0.0) or ((lvl_total_end / 1e3 * area) < volume_tolerance):
        logger.info("{}: {} - P_INO_FB Quantity in SOIL Store has gone negative, quantity reset "
                    "to zero.".format(waterbody, datetime_time_step))
        dict_states_wq['soil']['p_ino_fb'] = 0.0
    else:
    ...
    if (m_soil_new < 0.0) or ((lvl_total_end / 1e3 * area) < volume_tolerance):
        logger.info("{}: {} - P_ORG_RA Quantity in SOIL Store has gone negative, quantity reset "
                    "to zero.".format(waterbody, datetime_time_step))
    ...
    m_soil_new = m_soil * attenuation - 0.5 * dict_m_mobilised['pph'] + \
        (conversion_p_ino_ra_into_fb - conversion_p_ino_fb_into_ra) * time_factor
    if (m_soil_new < 0.0) or ((lvl_total_end / 1e3 * area) < volume_tolerance):
        logger.info("{}: {} - P_ORG_FB Quantity in SOIL Store has gone negative, quantity reset "
                    "to zero.".format(waterbody, datetime_time_step))
        dict_states_wq['soil']['p_org_fb'] = 0.0
    else:
